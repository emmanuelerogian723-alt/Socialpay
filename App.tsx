...
const App: React.FC = () => {
  ...
  // Load User Session
  useEffect(() => {
    const savedUserId = localStorage.getItem('socialpay_current_user_id');
    if (savedUserId) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
       const notes = storageService.getNotifications(user.id);
  ...
  const handleLogin = (u: User) => {
    setUser(u);
    localStorage.setItem('socialpay_current_user_id', u.id);
    setCurrentView('dashboard');
  };

  const handleLogout = () => {
    setUser(null);
    localStorage.removeItem('socialpay_current_user_id');
    setCurrentView('dashboard');
  };
